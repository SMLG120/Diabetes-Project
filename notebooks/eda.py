...
# Count plot
vc = train_df["diabetes"].value_counts()
...
# %%
num_cols = train_df.select_dtypes(include=[np.number]).columns.drop(["id", "diabetes"]).tolist()
...
for i, col in enumerate(num_cols):
    for label, color in [(0, "#4C72B0"), (1, "#DD8452")]:
        subset = train_df[train_df["diabetes"] == label][col].dropna()
...
# %%
corr_matrix = train_df[num_cols + ["diabetes"]].corr()
...
print(corr_matrix["diabetes"].drop("diabetes").abs().sort_values(ascending=False))
...
for i, col in enumerate(num_cols):
    sns.boxplot(
        data=train_df, x="diabetes", y=col,
...
class_balance = train_df["diabetes"].mean() * 100
print(f"  Class balance  : {class_balance:.1f}% positive (diabetes=1)")
